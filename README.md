...
| **neovim/**             | Stowable Neovim config ([nix-wrapper-modules](https://github.com/BirdeeHub/nix-wrapper-modules)): `flake.nix` + lua + `module.nix` under `.config/nvim` (`stow neovim` → `~/.config/nvim`). NixOS takes `neovim/.config/nvim` as the `nvim-config` input and installs via `getInstallModule`. `wrappers.neovim.liveLua` (default `true`) loads Lua from `~/.config/nvim` without rebuilds; set `liveLua = false` to bake the tree into the store. |
