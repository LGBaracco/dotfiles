...
| **neovim/**             | Stowable Neovim config ([nix-wrapper-modules](https://github.com/BirdeeHub/nix-wrapper-modules)): lua + `module.nix` live under `.config/nvim` (so `stow neovim` only populates `~/.config/nvim`). NixOS imports that module via `getInstallModule`; the wrapped binary uses a pure store copy of that tree (rebuild after lua/nix edits). |
